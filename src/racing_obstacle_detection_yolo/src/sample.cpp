...
class ObstacleDetectionNode : public hobot::dnn_node::DnnNode {
 ...
 private:
  ...
  rclcpp::Publisher<ai_msgs::msg::PerceptionTargets>::SharedPtr msg_publisher_ =
      nullptr;
  //节点消息订阅者
  rclcpp::Subscription<std_msgs::msg::Int32>::SharedPtr sign_sub =
      nullptr;
  ...
  //上位机消息回调
  void sign_sub_callback(const std_msgs::msg::Int32::SharedPtr data_msg);
};
...
ObstacleDetectionNode::ObstacleDetectionNode(const std::string& node_name,
  ...
  // 接收上位机信号
  sign_sub = this->create_subscription<std_msgs::msg::Int32>(
    "sign_foxglove", 10, std::bind(&ObstacleDetectionNode::sign_sub_callback, this, std::placeholders::_1));
}
...
void ObstacleDetectionNode::sign_sub_callback(const std_msgs::msg::Int32::SharedPtr data_msg){
  if (data_msg->data == 5) {
    this->flag = false;
  } else if (data_msg->data == 6) {
    this->flag = true;
  }
}
